...
public class DesktopPlatformSupport extends PlatformSupport {
	...
	public void saveScreenshot() {
		try {
			String baseStoragePath = FileUtils.getBaseStorageDir();
			if (baseStoragePath == null) {
			    // 处理无法获取基本存储目录的情况，例如回退到用户主目录
			    baseStoragePath = System.getProperty("user.home") + File.separator + "ShatteredPixelDungeon";
			    new File(baseStoragePath).mkdirs(); // 确保目录存在
			}
			File screenshotDir = new File(baseStoragePath, "screenshots");
			if (!screenshotDir.exists()) {
				screenshotDir.mkdirs();
			}

			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
			String fileName = "shattered_" + sdf.format(new Date()) + ".png";
			File screenshotFile = new File(screenshotDir, fileName);

			Pixmap pixmap = ScreenUtils.getFrameBufferPixmap(0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
			PixmapIO.writePNG(new FileHandle(screenshotFile), pixmap);
			pixmap.dispose();

			String message = Messages.get(this, "screenshot_saved", screenshotFile.getAbsolutePath());
