...
public class GameScene extends PixelScene {
	...
	// Add key listener for screenshot using com.watabou.input.KeyEvent
	KeyEvent.addKeyListener(new Signal.Listener<KeyEvent>() {
		@Override
		public boolean onSignal(KeyEvent event) { // <--- 返回类型修改为 boolean
			// Using F2 for screenshot (LibGDX key code for F2 is Input.Keys.F2)
			// We check for key press (event.pressed == true)
			if (event.pressed && event.code == Input.Keys.F2) {
		...
		// Touch controls remain unchanged
